...
//2 - D Dimensions Array(x, y)
	int             coordinates[2] = {0, 0};
//(x, y)
	coordinates for grid topology
	int             dim_wrapping[2]
		= {0, 1};
//To
do
	a cyclic shifting in the y dimension
	int             reorder = 1;
//Let virtual topology of grid be optimized to
// the physical topology by MPI for performance
	.
		MPI_Comm grid_comm;
//Communicator for total
	grid
		MPI_Comm row_comm;
...
MPI_Abort(MPI_COMM_WORLD, rc);
		...
		for             (int i = 1; i < argc; ++i) {
			if (strcmp(argv[i], "-o") == 0 or strcmp(argv[i], "--order") == 0) {
				...
				/*
				 * Copy command line argument into matrix
				 * order variable n which will then
				 * automatically be available to all
				 * processes
				 */
				n = atoi(argv[++i]);
			}
		...
		/* Distribute Data */
		double          p = num_proc;
		n_sub = n / sqrt(p);	/* We assume that number of procs is
					 * a perfect square and the n*n
					 * matrices are evenly divisble by
					 * sqrt(p) */
		...
		/*
		 * Cache Optimization Ensuring that all items are "touched"
		 * in C submatrix
		 */
...
void            makeGrid() {
	...
	/* Create a virtual 2D-grid topology */
	                ...
	                MPI_Comm_rank(grid_comm, &my_rank);	/* Note: use all_grid
								 * because we want the
								 * rank in grid */

	/* Create a communicator for each row */
	                MPI_Cart_coords(grid_comm, my_rank, num_dim, coordinates);
	                MPI_Comm_split(grid_comm, coordinates[0], coordinates[1], &row_comm);	/* x is the color and y
												 * the key */

	/* Create a communicator for each column */
	                MPI_Comm_split(grid_comm, coordinates[1], coordinates[0], &col_comm);	/* y is the color and x
												 * the key */
...
void            distributeData(int in_rank, int n_sub, int sublen, int p, double *submatrixA, double *submatrixB) {
	...
	MPI_Status      status;
	int             seed = SEED;
	int             len = n * n;	/* Total number of elements needed
					 * for matrix of order n */
	int             pos[2] = {0, 0};	/* Position for (x,y)
						 * coordinates */
	int             grid_rank;
	...
	if              (in_rank == 0) {
		...
		/*
		 * Only print out matrix A and B when order <= 16 ... to not
		 * print out very large matrices but still allow for testing
		 * accuracy with smaller matrices
		 */
		if              (n <= MAX_MATRIX_PRINT) {
			/*
			 * Print out the random array for A and B and test if
			 * we received this in the grid ok or not
			 */
			printf("*******************   A  Matrix ********************************** \n\n");
			for (int j = 0; j < len; j++) {
				printf("%f ", matrixA[j]);
		...
		/*
		 * Create a new MPI Vector Type to divide the array into
		 * blocks to transfer
		 */
		int             count = n_sub;
		int             blocklen = n_sub;
		...
		for (int i = 0; i < dimensions[0]; i++) {
			for (int j = 0; j < dimensions[1]; j++) {
				...
				/*
				 * Bit of manipulation to decide what section
				 * of array to send
				 */
				...
				if (j == dimensions[1] - 1)
					//we reached the last block in the row !
	...
	/*
	 * All other processes including process 0 will recieve their
	 * portions of main matrix into their sub matrix
	 */
...
void            foxMultiply(double p, int in_rank, double *submatrixA, double *submatrixB, double *submatrixC) {
	int             src;
	              //source of B block when shifting upwards
	int             dst;
	              //destination of B block when shifting upwards
	int             x;
	              //x - cordinate = row in grid
	int             y;
	              //y - cordinate = col in grid
	int             root = 0;
	int             len = n_sub * n_sub;
	int             grid_order = sqrt(p);
              //Order of the 2 - D grid:                Need this to shift A blocks in row !
	...
	/*
	 * Pre-compute the address for the circular shifting upwards of B
	 * matrix's blocks
	 */
	                src = (x + 1) % grid_order;
	                dst = (x - 1 + grid_order) % grid_order;
	              //Done in case we end up with a - ve number for x - 1

	/*
	 * Local storage for submatrix A which we will broadcast around and
	 * storage for submatrix B which we move upwards
	 */
	double         *temp_subA = (double *)
		calloc(len, sizeof(double));
	...
	for             (int k = 0; k < grid_order; k++) {
		/*
		 * Phase 1. Broadcast the diagonal blocks at each step
		 * calculating the new diagonal
		 */
		root = (x + k) % grid_order;
		//Compute the root of the broadcast
		/*
		 * When on diagonal --> Broadcast OUTWARDS to all other procs
		 * in row
		 */
		if (root == y) {
			...
			/*
			 * Have to receive the broadcast INCOMING from the
			 * root
			 */
		...
		/*
		 * Swap the Pointers now so that in the next step we are
		 * sending over temp_subB
		 */
		double         *tmp;
		tmp = submatrixB;
...
void            collectData(double p, int in_rank, double *subC) {
	...
	/*
	 * Create a new MPI Vector Type to divide the array into blocks to
	 * transfer
	 */
	int             count = n_sub;
	int             blocklen = n_sub;
	...
	if              (in_rank == 0) {
		...
		for (int i = 0; i < p; i++) {
			/*
			 * Probe and see if the incoming source rank is the
			 * one we expect
			 */
